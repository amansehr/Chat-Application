...
import socketio
from db import add_room_members, get_room_members, get_rooms_for_user, get_room, get_user, is_room_member, remove_room_members, save_message, save_user, save_room, is_room_admin, update_room, get_messages
from pymongo.errors import DuplicateKeyError

app = Flask(__name__)
socketio = SocketIO(app)
app.secret_key = "youcannotdecodethis"
...
def handle_join_room_event(data): 
    ...
    room = data["room"]
    app.logger.info(f"{newuser} has joined the room {room}")
    join_room(room)
...
def handle_send_message_event(data):
    ...
    app.logger.info(f"{user} has sent message to the room {room} : {message}")
...
def handle_leave_room_event(data):
    user = data["username"]
    room = data["room"]
    app.logger.info(f"{user} has left the room {room}")
    leave_room(room)
